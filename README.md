...
6.  **背景透明化**
    将提取出的核心纹饰的黑色背景转换为透明的 Alpha 通道，并保存为 PNG 格式。

## 4. 如何运行项目

### 4.1. 环境配置

1.  **准备环境**: 确保已安装 Python 3.8+ 和 Conda。
2.  **克隆代码**:
    ```bash
...
4.  **安装依赖**:
    ...
    ```
5.  **下载模型**:
    下载 U-2-Net 预训练模型 (`u2net.pth`)，并将其放置在 `saved_models/u2net/` 目录下。
...
如果您需要处理大量的本地图片，可以直接使用命令行的批处理脚本。

1.  将待处理图片放入文件夹（例如 `my_images/`）。
2.  执行以下命令：
...
**参数说明:**
  ...
  * `--images`: 指定包含原始图片的输入目录。
  * `--output`: 指定用于存放所有处理结果的输出目录。

处理完成后，所有提取出的纹饰图片将保存在 `./my_results/final_output/` 目录下。
